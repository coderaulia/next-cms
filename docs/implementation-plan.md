...
**Tasks:**
- [x] Add rate limiting to both analytics routes
- [x] Add max-length validation on all string fields (visitorId, sessionId, referrer, utm* — cap at 512 chars)
- [x] Consider moving `/api/analytics/event` behind admin auth if only admin dashboards call it
...
**Tasks:**
- [x] Add `TRUSTED_PROXY_COUNT` env var (document in `.env.example`)
- [x] Rewrite `getClientIdentifier()` to use trusted-proxy-aware IP extraction
- [x] Update login lockout in `adminAuth.ts` to use same function
- [x] Test with `TRUSTED_PROXY_COUNT=0` (direct) and `=1` (behind proxy)
...
**Tasks:**
- [x] Implement `validateFileMagicBytes()` in `mediaStorage.ts`
- [x] Block SVG uploads entirely (remove `image/svg+xml` from allowed MIME if present) or add sanitization step
- [x] Verify max file size is checked server-side (not just client-side)
- [x] Add file extension allowlist check as secondary guard
...
**Tasks:**
- [x] Add `truncate()` helper to both analytics routes
- [x] Apply length caps to all string fields before DB insert
...
**Tasks:**
- [x] Audit all admin API routes — list which use `assertAdminRequest` vs `assertAdminPermission`
- [x] Replace `assertAdminRequest` with role-gated `assertAdminPermission` calls where appropriate
- [x] Add `content:read` permission to contact-submissions GET handler
...
**Tasks:**
- [x] Add `PASSWORD_PEPPER` env var (32+ byte hex, document in `.env.example`)
- [x] Update `hashAdminPassword()` and `verifyAdminPassword()` to apply pepper
- [x] Decide migration strategy: force password reset vs transparent re-hash on login
...
**Tasks:**
- [x] Add warning log when fallback activates
- [x] Cap fallback store at N entries (evict oldest)
- [x] Add note to `CLAUDE.md` / ops runbook
...
**Tasks:**
- [x] Add `logoutAllSessions()` to `adminAuth.ts`
- [x] Add `POST /api/admin/auth/logout-all` route (requires auth)
...
**Tasks:**
- [x] Add a single-line comment explaining the OR logic intent
...
**Tasks:**
- [x] Add `X-XSS-Protection: 1; mode=block` to security headers in `middleware.ts`
...
**Tasks:**
- [x] Ensure catch block logs only the error message, not request headers
- [x] Scrub Authorization header from any error objects before logging
